...
const screenForNames = (arr) => {
  ...
  let array = arr.filter((y) => y.match(x));
  return array;
};
...
const toTitleCase = (arr) => {
  let array = arr.map(str => str.charAt(0).toUpperCase()+ str.slice(1));
  return array;
};
...
let biggerThanLuke = (arr) => {
  let array = arr.filter((character)=>character.mass>77).map((character)=> character.name).join(' - ');
  return array;
};
