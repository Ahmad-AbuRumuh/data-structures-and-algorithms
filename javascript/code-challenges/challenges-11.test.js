...
function transformToLis(obj){
  let array = Object.entries(obj).map(element => `<li>${element.join(': ')}</li>`);
  return array;
}
...
const count = (target, input) => {
  let array = [];
  input.forEach(val => {
    array.push(val.filter(element => element === target));
  });
  let result = array.toString().split(',').join('').length;
  return result;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let array = input.map(element => element.filter(element => typeof element === 'number' && element % 5 === 0 ? true : false).map(num => Math.pow(2, num)));
  return array;
};
...
let findMaleAndFemale = (data) => {
  ...
  let result = array.map(element => element['name']).join(' and ');
  return result;
};
