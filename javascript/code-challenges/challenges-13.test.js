...
const longestString = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (stringArr.length > stringLength) {
      stringLength = stringArr.length;
      index = i;
    }
  }
  if (arr.length === 0)
...
const standardizePhoneNumbers = (arr) => {
  let tempArr = arr.map(item => `${item.substring(1, 4)}${item.substring(6, item.length).split('-').join('')}`);
  return tempArr;
};
...
const onlyOddChars = (str) => {
  ...
  let result = strArr.filter((item,index)=>{
    if(index%2 !== 0)
      return item;
  });
  let finalResult = '';
